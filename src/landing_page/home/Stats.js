import React from "react";
function Stats() {
  return (
    <>
      <div className="container mt-5 mb-5">
        <div className="row mt-5">
          <div className="col-6 align-self-center">
            <h1 className="mb-5">Trust with confidence</h1>
            <h2 className="fs-2">Customers-first always</h2>
            <p className="text-muted">
              That's why 1.3+ crore customers tust Zerodha with &#8377;3.5+ lakh
              crores worth of equality investments.
            </p>

            <h2 className="fs-2">No spam or gimmicks</h2>
            <p className="text-muted">
              No gimmicks, spam, "gamification", or annoying push notifications.
              High quality apps that you use at your pace, the way you like.
            </p>

            <h2 className="fs-2">The Zerodha universe</h2>
            <p className="text-muted">
              Not just an app, but a whole ecosystem Our investments in 30+
              fintech startups offer you tailored services to your tailored
              services specific to your needs.
            </p>

            <h2 className="fs-2">Do better with money</h2>
            <p className="text-muted">
              With initiatives like Nudge and Kill Switch, we don't just
              facilitate transactions, but actively help you do better with your
              money.
            </p>
          </div>
          <div className="col-6 mb-5">
            <img src="media\images\ecosystem.png" style={{ width: "90%" }} />
            <div className="text-center">
              <a href="" className="mx-5" style={{ textDecoration: "none" }}>
                Explore our products <i class="fa-solid fa-arrow-right"></i>
              </a>
              <a href="" style={{ textDecoration: "none" }}>
                Try Kite <i class="fa-solid fa-arrow-right"></i>
              </a>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}

export default Stats;
